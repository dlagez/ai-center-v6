...
def _build_summary(doc_dict: dict, blocks: list[DoclingBlockPreview]) -> dict:
    ...
    return {
        ...
        "text_blocks": sum(1 for label in labels if label in {"text", "paragraph", "title", "section_header", "page_header", "page_footer"}),
...
async def parse_docling_pdf(
    ...
    try:
        file_record = service.get_pdf_file(request.file_id)
